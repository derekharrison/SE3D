...
void solver(d_data domain_data,
    ...
    /* Parameters */
    n_r = domain_data.n_r;
    n_theta = domain_data.n_theta;
    n_phi = domain_data.n_phi;
    nt = domain_data.nt;
    R = domain_data.R;
    ...
    for(int i = 0; i < n_r; ++i) {
        for(int j = 0; j < n_theta; ++j) {
            for(int k = 0; k < n_phi; ++k) {
                solver_data->psi[i][j][k].a = psi[i][j][k].a;
                solver_data->psi[i][j][k].b = psi[i][j][k].b;
                solver_data->prob_density[i][j][k].a = prob_density[i][j][k].a;
                solver_data->prob_density[i][j][k].b = prob_density[i][j][k].b;
            }
        }

        solver_data->psi_p_top[i].a = psi_p_top[i].a;
        solver_data->psi_p_top[i].b = psi_p_top[i].b;
        solver_data->psi_p_bottom[i].a = psi_p_bottom[i].a;
        solver_data->psi_p_bottom[i].b = psi_p_bottom[i].b;
        solver_data->prob_density_p_top[i].a = prob_density_p_top[i].a;
        solver_data->prob_density_p_top[i].b = prob_density_p_top[i].b;
    ...
    free_mat3D(psi, n_r, n_theta);
    free_mat3D(psi_prev, n_r, n_theta);
    free_mat3D(psio, n_r, n_theta);
    free_mat3D(prob_density, n_r, n_theta);
    delete [] psi_p_top;
    delete [] psi_p_bottom;
    delete [] psi_p_topo;
    delete [] psi_p_bottomo;
